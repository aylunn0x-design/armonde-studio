...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/admin') {
    return send(res, 200, adminLayout(`
      ...
      <section class="stack"><div class="panel"><div class="top"><div><h1>Showcase Projects</h1><p class="muted">Visual management for the public portfolio and homepage.</p></div><a class="btn" href="/admin/new-project">+ Add Project</a></div><div class="project-grid">${content.projects.map((p, i) => `<div class="project-card"><img src="${h(p.image || '')}" alt="${h(p.title || '')}"/><div class="project-body"><div class="toolbar"><span class="pill">${h(p.category || 'Project')}</span><span class="status-pill ${p.status === 'published' ? 'published' : 'draft'}">${h(p.status || 'draft')}</span>${p.featured ? '<span class="pill">Featured</span>' : ''}</div><h3 style="margin-top:12px">${h(p.titleEn || p.title || '')}</h3><p class="mini">${h(p.location || '')}${p.year ? ' · ' + h(p.year) : ''}</p><p>${h(p.descriptionEn || p.description || '')}</p><div class="toolbar"><a class="btn secondary" href="/admin/edit-project?id=${i}">Edit</a><a class="btn secondary" href="/project/${slugify(p.titleEn || p.title)}" target="_blank">View Public</a><form method="POST" action="/admin/delete-project" style="display:inline"><input type="hidden" name="id" value="${i}"/><button class="btn danger" type="submit">Delete</button></form></div></div></div>`).join('')}</div></div></section></div>
    `));
  }
...
function projectComposer(title, action, project, id='') {
  ...
  return `
    ...
    <div class="helper-box"><strong>How this works</strong><p class="mini">1) Add a cover image 2) fill in bilingual titles/descriptions 3) paste extra gallery image links line-by-line 4) choose published or draft.</p></div>
