...
function shell(title, body) {
  ...
  *{box-sizing:border-box} body{font-family:Inter,system-ui,sans-serif;margin:0;background:linear-gradient(180deg,#0b0b0d,#101114);color:var(--text)} a{text-decoration:none;color:inherit} img{display:block;max-width:100%}
  ...
  .grid{display:grid;gap:20px}.grid-2{grid-template-columns:1fr 1fr}.stat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.stat-card{padding:18px;border-radius:20px;background:rgba(255,255,255,.03);border:1px solid var(--line)}.cards{display:grid;gap:18px}.card,.panel,.sidebar,.login-card,.project-card,.glass{background:rgba(255,255,255,.03);border:1px solid var(--line);border-radius:24px;box-shadow:0 22px 60px rgba(0,0,0,.28)}.card,.panel{padding:24px}.glass{padding:26px;backdrop-filter:blur(10px)}
...
function publicHome(content) {
  ...
  return shell(content.siteName, `
    ...
    <main>
      ...
      <section class="section"><div class="wrap stat-grid"><div class="stat-card"><h3>Residential</h3><p class="mini">Private homes, villas, calm luxury living.</p></div><div class="stat-card"><h3>Commercial</h3><p class="mini">Boutique business spaces with premium identity.</p></div><div class="stat-card"><h3>Interior</h3><p class="mini">Materials, atmosphere, and refined spatial styling.</p></div></div></section><section id="about" class="section soft"><div class="wrap grid grid-2"><div><p class="mini">About the studio</p><h2>Architecture that feels calm, expensive, and unforgettable.</h2></div><div class="glass"><p>${h(content.about)}</p></div></div></section>
...
function publicProjectPage(content, project) {
  ...
  return shell(`${project.titleEn || project.title} — ${content.siteName}`, `
    ...
    <main>
      ...
      <section class="section"><div class="wrap grid grid-2"><div class="glass"><p class="mini">Project Details</p><h2>${h(project.titleEn || project.title)}</h2><p><strong>Client:</strong> ${h(project.client || 'Private Client')}<br/><strong>Area:</strong> ${h(project.area || '')}<br/><strong>Category:</strong> ${h(project.category || '')}</p><p>${h(project.concept || project.descriptionEn || project.description || '')}</p></div><div class="glass"><p class="mini">Design Notes</p><p>This project was developed around spatial calm, strong material identity, and a restrained luxury language intended to feel timeless rather than trend-based.</p></div></div></section><section class="section soft"><div class="wrap"><div class="top"><div><p class="mini">Gallery</p><h2>Project visuals</h2></div></div><div class="gallery-grid">${gallery.map(img => `<div class="gallery-item"><img src="${h(img)}" alt="gallery"/></div>`).join('')}</div></div></section>
    </main>
  `);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/admin/edit-project') {
    ...
    const gallery = (params.get('gallery') || '').split('\n').map(s => s.trim()).filter(Boolean);
    const image = params.get('image') || gallery[0] || '';
    content.projects[id] = { title: params.get('titleEn') || params.get('title') || '', titleMn: params.get('titleMn') || '', titleEn: params.get('titleEn') || '', category: params.get('category') || '', year: params.get('year') || '', location: params.get('location') || '', description: params.get('descriptionEn') || params.get('description') || '', descriptionMn: params.get('descriptionMn') || '', descriptionEn: params.get('descriptionEn') || '', image, gallery: gallery.length ? gallery : [image].filter(Boolean), featured: params.get('featured') === 'on', status: params.get('status') || 'draft', client: params.get('client') || 'Private Client', area: params.get('area') || '', concept: params.get('concept') || params.get('descriptionEn') || '' };
    writeContent(content); res.writeHead(302, { Location: '/admin' }); return res.end();
  }
...
function projectComposer(title, action, project, id='') {
  ...
  return `
    ...
    <form class="composer" method="POST" action="${action}">${id !== '' ? `<input type="hidden" name="id" value="${id}"/>` : ''}
      ...
      <div class="row"><div><label>Location</label><input name="location" value="${h(project.location || '')}" placeholder="Ulaanbaatar"/></div><div><label>Year</label><input name="year" value="${h(project.year || '')}" placeholder="2026"/></div></div><div class="row"><div><label>Client</label><input name="client" value="${h(project.client || '')}" placeholder="Private Client"/></div><div><label>Area</label><input name="area" value="${h(project.area || '')}" placeholder="420 m²"/></div></div><div><label>Concept Summary</label><textarea name="concept" rows="4" placeholder="Short high-level concept summary...">${h(project.concept || '')}</textarea></div>
